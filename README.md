...
- **Creates or updates** `CHANGELOG.md`. Existing entries are never rewritten: new sections are inserted in version order, released sections are left alone, and running it twice changes nothing.
- **Finds what is missing on its own.** Each tag (`v1.2.3`, `1.2.3`, `pkg@1.2.3`) is a release; tags without a section are added, plus the commits after the last tag. Those get the version in `package.json` if it is not tagged yet, otherwise `Unreleased`. That section is **extended, not replaced**: when you commit more work and run the command again, only the new commits are added to it (a hand-written section is completed the same way, skipping what it already says). Its heading is renamed when `Unreleased` becomes a version. To know where it stopped, gitowl keeps an invisible `<!-- gitowl:abc1234 -->` comment under that heading; it does not show when rendered and you can delete it (gitowl then falls back to comparing wording).
